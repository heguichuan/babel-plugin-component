  ...
  return ({ types }) => {
    ...
    function importMethod(methodName, file, opts) {
      if (!selectedMethods[methodName]) {
        ...
        const {
          fallbackLibraryName = defaultLibraryName, 
          libDir = 'lib',
          style = true,
          styleLibrary,
          root = '',
        ...
        let libraryName = options.libraryName || defaultLibraryName;
        ...
        // 组件库降级处理
        if (libraryObjs[methodName] || components.indexOf(parseName(methodName, camel2Dash)) !== -1) {
    ...
    return {
      visitor: {
        Program(path, { opts }) {
          ...
          const componentsFilePath = resolve(process.cwd(), `node_modules`, opts.componentsFilePath || 'onenet-ui/components.json');

          if (components === null || typeof components == 'undefined') {
            let compFile;
            try {
                compFile = JSON.parse(readFileSync(componentsFilePath, 'UTF-8'));
            } catch (error) {
                throw Error('组件列表获取失败：\n' + componentsFilePath);
            }
            components = Object.keys(compFile || {})
          }
        },
